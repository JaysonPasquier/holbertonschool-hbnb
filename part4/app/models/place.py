...
class Place(BaseModel):
    ...
    def add_amenity(self, amenity):
        ...
        if hasattr(self, 'amenities'):
            # Check if amenities is a list (in-memory model)
            if isinstance(self.amenities, list):
                if amenity not in self.amenities:
                    self.amenities.append(amenity)
            # Check if it's an SQLAlchemy relationship
            elif hasattr(self.amenities, 'append'):
                self.amenities.append(amenity)
            else:
                # If we're here, amenities might be something unexpected
